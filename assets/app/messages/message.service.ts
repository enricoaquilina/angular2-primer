...
export class MessageService{
    messages: Message[] = [];
    isEdit = new EventEmitter<Message>();
    ...
    addMessage(message: Message){
        ...
        return this._http.post('http://localhost:3000/message'+token, body, {headers:headers})
            .map(response => {
                 ...
                 let message = new Message(data.content, data.user.firstName, data._id, data.user._id);
                 return message;
            })
            ...
            .map(response => {
                ...
                for (let i =0; i< data.length; i++){
                    let message = new Message(
                        data[i].content, data[i].user.firstName, data[i]._id, data[i].user._id
                    )
                    objs.push(message);
                }
                return objs;
